...
<?php foreach ($generator->indices as $index): ?>
        $this->createIndex('<?= $generator->generateIndexName($index) ?>', '<?= $index->table ?>', <?= Generator::processArray(explode(';', $index->columns), '\'', false, true) ?><?= isset($index->unique) && $index->unique ? ', true' : '' ?>);
<?php endforeach ?>
<?php if (!empty($generator->indices) && !empty($generator->foreignKeys)): ?>

<?php endif ?>
<?php foreach ($generator->foreignKeys as $fKey): ?>
        $this->addForeignKey('<?= $generator->generateForeignKeyName($fKey) ?>', '<?= $fKey->table ?>', <?= Generator::processArray(explode(';', $fKey->columns), '\'', false, true) ?>, '<?= $fKey->refTable ?>', <?= Generator::processArray(explode(';', $fKey->refColumns), '\'', false, true) ?><?= isset($fKey->delete) ? ", '{$fKey->delete}'" : '' ?><?= isset($fKey->update) ? ((isset($fKey->delete) ? '' : ', null') . ", '{$fKey->update}'") : '' ?>);
<?php endforeach ?>
    }
